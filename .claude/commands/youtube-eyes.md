...
- Locate `transcript_cli.py`, `video_frames_cli.py`, and `pedagogic.md` in the current project.
...
- If `video_frames_cli.py` is missing, warn but continue without frame extraction (degrade to transcript-only mode).
...
- **Code frames** (`type: "code"`): prefer using the `ocr_text` to create a fenced code block with the appropriate language tag (e.g. ` ```zig`, ` ```c`). If the OCR text is garbled, incomplete, or clearly wrong, embed the screenshot image instead: `![description](frame-NNNNs.png)`. Clean up obvious OCR artifacts in code (misread characters, missing indentation) when the correct code is clear from context.
- **Diagram frames** (`type: "diagram"`): embed the screenshot image directly: `![description](frame-NNNNs.png)`. If the diagram structure is simple enough (e.g. a few boxes with arrows), you may recreate it as ASCII art or a Markdown list instead, but only if you can do so faithfully.
- **Slide frames** (`type: "slide"`): use `ocr_text` when the slide is primarily text (bullet points, titles). Embed the screenshot image only if the slide has visual layout or formatting that plain text cannot capture. Skip title slides or slides that just repeat section headings already in the article.
